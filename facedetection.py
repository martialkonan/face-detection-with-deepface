# -*- coding: utf-8 -*-
"""Copie de colab_webcam.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1UmPhr3Q2gAs-GIMFiOM51L7ZMuFq0znH
...
def video_stream():
  js = Javascript('''
    ...
    async function createDom() {
      ...
      const modelOutEmotion = document.createElement('div');
      modelOutEmotion.innerHTML = "<span>Emotion:</span>";
      emotionElement = document.createElement('span');
      emotionElement.innerText = 'No data';
      emotionElement.style.fontWeight = 'bold';
      modelOutEmotion.appendChild(emotionElement);
      div.appendChild(modelOutEmotion);
    ...
    async function stream_frame(label, imgData, emotion) {
      if (shutdown) {
        removeDom();
        shutdown = false;
        return '';
...
def video_frame(label, bbox, emotion):
  data = eval_js('stream_frame("{}", "{}","{}")'.format(label, bbox, emotion))
  return data
...
count = 0 
while True:
    js_reply = video_frame(label_html, bbox, emotion_t)
    if not js_reply:
        break
#try:
  # convert JS response to OpenCV Image
    ...
    try:
      ...
      emotion_t = predict['dominant_emotion'] 
      position = (0,50)
      faceExpr = emotion_t
      gender = predict['gender']
      fileName = 'OUTPUT_'+gender+'_'+faceExpr+'.jpg'
      font = cv2.FONT_HERSHEY_SIMPLEX
      if(emotion_t == 'happy'): fontColor = (0, 153, 51, 0)
      elif(emotion_t == 'sad' or emotion_t == 'fear'): fontColor = (204, 51, 0, 0)
      else: fontColor = (0, 102, 255, 0)
      cv2.putText(
          ...
          3, cv2.LINE_4) #font stroke
      bbox_array = cv2.putText(
          bbox_array, #numpy array on which text is written
          emotion_t,
          (10,50), #position at which writing has to start
          cv2.FONT_HERSHEY_SIMPLEX, #font family
          1, #font size
          fontColor, #font color
          1, cv2.LINE_4) #font stroke
      cv2.imwrite(fileName, img)    
    except: plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB)) 
    # ============================================== #
